...
type SyncingInboxInstance = {
  instance: InboxInstance;
  historySyncJob: InboxHistorySyncJob;
};
...
function getInboxHistorySyncJob(
  instance?: Pick<InboxInstance, "providerMetadata"> | null,
) {
  ...
  return {
    ...
    messagesProcessed: readCount(historySyncJob.messagesProcessed),
    inboundMessages: readCount(historySyncJob.inboundMessages),
    outboundMessages: readCount(historySyncJob.outboundMessages),
    mediaMessages: readCount(historySyncJob.mediaMessages),
  } satisfies InboxHistorySyncJob;
}
...
export function InboxPageContent({
  ...
  const inboxInstancesQuery = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const hasSyncing =
        Array.isArray(data) &&
        data.some((instance) => getInboxHistorySyncJob(instance)?.status === "RUNNING");
  ...
  const syncingInstances = useMemo(
    () =>
      inboxInstances.flatMap((instance) => {
        const historySyncJob = getInboxHistorySyncJob(instance);
        ...
        return [
          {
            instance,
            historySyncJob,
          } satisfies SyncingInboxInstance,
        ];
      }),
    [inboxInstances],
  );
  ...
  const conversations = useMemo(() => {
    ...
    const latestConversationsById = new Map<string, Conversation>();

    for (const conversation of list) {
      const current = latestConversationsById.get(conversation.id);

      if (!current) {
        latestConversationsById.set(conversation.id, conversation);
        continue;
      }

      const shouldReplaceCurrent =
        getConversationSortTimestamp(conversation) >=
        getConversationSortTimestamp(current);
      const preferredConversation = shouldReplaceCurrent ? conversation : current;
      const fallbackConversation = shouldReplaceCurrent ? current : conversation;

      latestConversationsById.set(conversation.id, {
        ...fallbackConversation,
        ...preferredConversation,
        contactAvatarUrl:
          preferredConversation.contactAvatarUrl ??
          fallbackConversation.contactAvatarUrl ??
          null,
        contactDisplayPhone:
          preferredConversation.contactDisplayPhone ??
          fallbackConversation.contactDisplayPhone ??
          null,
        lastMessagePreview:
          preferredConversation.lastMessagePreview ??
          fallbackConversation.lastMessagePreview ??
          null,
      });
    }

    return Array.from(latestConversationsById.values()).sort(
      (left, right) =>
        getConversationSortTimestamp(right) - getConversationSortTimestamp(left),
    );
                ...
                <div className="mt-3 flex items-start gap-2.5 rounded-2xl border border-primary/20 bg-primary/[0.04] px-3.5 py-2.5">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-0.5 space-y-1 text-[11px] text-muted-foreground">
                      {syncingInstances.map(({ instance, historySyncJob }) => (
                        <p key={instance.id} className="truncate">
                          <span className="font-medium text-foreground/80">
                            {instance.name}:
                          </span>{" "}
                          {historySyncJob.detail ??
                            "As conversas e mensagens aparecem conforme chegam."}
                        </p>
                      ))}
                    </div>
                  </div>
