            ...
            <Card>
              <CardContent className="flex min-h-72 flex-col items-center justify-center text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Crie bases para estruturar conhecimento interno e futuras buscas semanticas.
                </p>
              </CardContent>
            </Card>
          ) : null}
        ...
        <Card className="p-0">
          <CardHeader className="flex items-center justify-between gap-4 p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {detail?.description ??
                  'Ao selecionar uma base, os documentos aparecem aqui.'}
              </p>
            </div>
            {detail ? (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        await apiRequest(`knowledge-documents/${document.id}`, {
                          method: 'DELETE',
                        });
                        await queryClient.invalidateQueries({
                          queryKey: ['knowledge-base-detail', selectedBase?.id],
      ...
      <Dialog open={baseDialogOpen} onOpenChange={setBaseDialogOpen}>
        <DialogContent className="sm:h-[calc(100dvh-1.5rem)] sm:max-h-[calc(100dvh-1.5rem)]">
          <DialogHeader className="shrink-0 pr-10">
            ...
            <DialogDescription>
              Estruture conhecimento interno com status e tipo definidos.
            </DialogDescription>
          </DialogHeader>
          <KnowledgeBaseForm
      ...
      <Dialog open={docDialogOpen} onOpenChange={setDocDialogOpen}>
        <DialogContent className="sm:h-[calc(100dvh-1.5rem)] sm:max-h-[calc(100dvh-1.5rem)]">
          <DialogHeader className="shrink-0 pr-10">
            ...
            <DialogDescription>
              Cadastre texto ou URL para uso futuro em embeddings e assistentes.
            </DialogDescription>
          </DialogHeader>
          <KnowledgeDocumentForm
            key={selectedBase?.id ?? 'new-doc'}
            onSubmit={(values) => createDocMutation.mutate(values)}
          />
        </DialogContent>
      </Dialog>
    </div>
...
function KnowledgeDocumentForm({
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex shrink-0 flex-col-reverse gap-2.5 border-t border-border pt-4 sm:flex-row sm:justify-end">
