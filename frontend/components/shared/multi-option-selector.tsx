...
export function MultiOptionSelector({
  ...
  if (!options.length) {
    return (
      <div className="rounded-[22px] border border-dashed border-white/10 bg-white/[0.02] px-4 py-4 text-sm text-muted-foreground">
        {emptyMessage}
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-2 rounded-[22px] border border-white/8 bg-background-panel/80 p-3 sm:grid-cols-2">
      {options.map((option) => {
